...
void CInventoryManagerDlg::ShowTabPage(int idx)
{
    ...
    GetDlgItem(IDC_BUTTON2)->ShowWindow(showInventory ? SW_SHOW : SW_HIDE); // 삭제 버튼
    GetDlgItem(IDC_BUTTON3)->ShowWindow(showInventory ? SW_SHOW : SW_HIDE); // 품목 추가 버튼
    ...
    // 설정 다이얼로그 보이기/숨기기
    if (m_pSettingsDlg) {
        m_pSettingsDlg->ShowWindow(showSettings ? SW_SHOW : SW_HIDE);
        if (showSettings) {
            // 메인 다이얼로그의 현재 기준값을 설정 다이얼로그로 전달
